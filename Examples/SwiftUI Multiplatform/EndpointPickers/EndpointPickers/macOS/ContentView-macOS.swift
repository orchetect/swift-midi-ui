...
struct ContentView: View {
    ...
    private var midiInConnectionView: some View {
        ...
        GroupBox(label: Text("MIDI In Connection")) {
            ...
            .updatingInputConnection(withTag: MIDIHelper.Tags.midiInConn)
    ...
    private var midiOutConnectionView: some View {
        GroupBox(label: Text("MIDI Out Connection")) {
            ...
            .updatingOutputConnection(withTag: MIDIHelper.Tags.midiOutConn)
            // the picker view requires injecting the observable MIDI manager into the environment
            .environment(\.midiManager, midiHelper.midiManager)
            .padding([.leading, .trailing], 60)
    ...
    private var eventLogView: some View {
        GroupBox(label: Text("Received Events")) {
            let events = midiHelper.receivedEvents.reversed().prefix(100)
